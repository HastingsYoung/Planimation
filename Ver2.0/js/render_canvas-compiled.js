...
var Component = function () {
    ...
    _createClass(Component, [{
        ...
        value: function bindClass(className) {
            if (!className) throw new Error("No class name entered!");
            var node = this.node;
            node.component = this;
    ...
    _createClass(Action, [{
        ...
        value: function renderModal(parentNode) {
            ...
            for (var t in obj.data.preconditions) {
                preconditions += "<span>" + obj.data.preconditions[t].state + "(" + obj.data.preconditions[t].operators.join(",") + "): " + obj.data.preconditions[t].predicate + "</span>";
            }

            for (var t in obj.data.effects) {
                effects += "<span>" + obj.data.effects[t].state + "(" + obj.data.effects[t].operators.join(",") + "): " + obj.data.effects[t].predicate + "</span>";
            }

            var nodeContent = "<div class='action'>" + "<header><h3 class='name'>Action Name: " + obj.data.name + "</h3></header><div class='content'><div class='tags'>" + "<div class='sub_header'><h5>Parameters: </h5>" + parameters + "</div><div class='sub_header'><h5>Preconditions: </h5>" + preconditions + "</div>" + "<div class='sub_header'><h5>Effects: </h5>" + effects + "</div>" + "</div><div class='description'><h5>Description: </h5><form action=''>" + "</form></div></div></div>";
            if (parentNode) parentNode.appendChild(nodeContent);else if (this.modalSelector) document.querySelector(this.modalSelector).appendChild(this.parseDom(nodeContent));
        }
    }]);
    ...
    _createClass(Predicate, [{
        ...
        value: function renderModal(parentNode) {
            ...
            for (var t in obj.data.rules) {
                rules += "<span>" + obj.data.rules[t].state + "(" + obj.data.rules[t].operators.join(",") + "): [default]" + obj.data.rules[t].default + "</span>";
            }

            var nodeContent = "<div class='predicate'>" + "<div class='content'><div class='tags'>" + "<div class='sub_header'><h5>Rules: </h5>" + rules + "</div>" + "</div><div class='description'><h5>Description: </h5><form action=''>" + "</form></div></div></div>";
            if (parentNode) parentNode.appendChild(nodeContent);else if (this.modalSelector) document.querySelector(this.modalSelector).appendChild(this.parseDom(nodeContent));
        }
    }]);
